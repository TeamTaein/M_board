...
<table class="type09">
	...
	<tr>
		...
		<u:isLogin>
		<a href="list.do">[목록]</a>
		<a href="${ctxPath }/logout.do" class="text-danger">[로그아웃하기]</a>		
		</u:isLogin>
		</td>		
	</tr>	
	...
	<c:if test="${articlePage.hasArticles() }">
		<li class="page-item">
			<c:if test="${articlePage.startPage > 5 }">
				<a class="page-link" href="search.do?pageNo=${articlePage.startPage - 5 }">이전</a>
			</c:if>		
		</li>			
			<c:forEach var="pNo" begin="${articlePage.startPage }" end="${articlePage.endPage }">
			<li class="page-item">	<a class="page-link" href="search.do?pageNo=${pNo }">${pNo }</a></li>
			</c:forEach>		
		<li class="page-item">	
			<c:if test="${articlePage.endPage < articlePage.totalPages }">
				<a class="page-link" href="search.do?pageNo=${articlePage.startPage + 5 }">다음</a>
			</c:if>			
		</li>
		</c:if>
	</ul>
